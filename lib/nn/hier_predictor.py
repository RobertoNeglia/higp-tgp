...
class HierPredictor(Predictor):
    def __init__(
        ...
        *,
        ...
        self.levels = levels
        super(HierPredictor, self).__init__(
            model=model,
    ...
    def _set_metrics(self, metrics):
        ...
        self.hier_train_metrics = nn.ModuleList(
            [
                MetricCollection(
        ...
        self.hier_val_metrics = nn.ModuleList(
            [
                MetricCollection(
                    metrics={k: self._check_metric(m) for k, m in metrics.items()},
                    prefix="val_",
        ...
        self.hier_test_metrics = nn.ModuleList(
            [
                MetricCollection(
                    metrics={k: self._check_metric(m) for k, m in metrics.items()},
                    prefix="test_",
    ...
    def update_and_log_hier_metrics(
        self, metrics: nn.ModuleList, ys_hat, ys, mask, **kwargs
    ):
        masks = [
            None,
        ] * (self.levels - 1) + [
    ...
    def shared_step(self, batch, batch_idx):
        ...
        y_loss, y_hat_loss, C, sizes, mask, reg_losses = self.predict_batch(
            batch,
            preprocess=False,
    ...
    def compute_loss(self, pred, target, mask, sizes, batch_size, name):
        ...
        if self.fit_hier_levels:
            for i in range(len(sizes) - 1):
                ...
                self.log_loss(
                    f"{name}_level_{len(sizes) - 1 - i}", loss_i, batch_size=batch_size
                )
        ...
        self.log_loss(f"{name}_base", loss_base, batch_size=batch_size)
        loss_base *= loss_w
        loss += loss_base
        return loss

    def training_step(self, batch, batch_idx):
        (y, y_loss), (y_hat, y_hat_loss), C, sizes, mask, reg_losses = self.shared_step(
            batch, batch_idx
        )
        ...
        if y_hat_loss.dim() == 5:
            for i, pred in enumerate(list(y_hat_loss)):
                loss += self.compute_loss(
                    pred, y_loss, mask, sizes, batch.batch_size, name=f"train_loss_{i}"
                )
            loss /= y_hat_loss.size(0)
        else:
            loss += self.compute_loss(
                y_hat_loss, y_loss, mask, sizes, batch.batch_size, name=f"train_loss"
            )
        ...
        if self.fit_hier_levels:
            for i, l_reg in enumerate(reg_losses):
                ...
                self.log_loss(f"reg_loss_{i}", l_reg, batch_size=batch.batch_size)
        ...
        if self.fit_hier_levels:
            if not self.reconcile_forecasts:
                ...
                coherence_loss = torch.norm(coherence_loss, p=2, dim=-1).mean()
            else:
                assert y_hat_loss.dim() == 5
                assert y_hat.dim() == 5
                coherence_loss = torch.norm(
                    y_hat_loss[0] - y_hat_loss[1], p=2, dim=-1
                ).mean()
        ...
        if self.model.hierarchy_builder.pooler_name == "gumbel":
            t = self.model.hierarchy_builder.pooling_layers[0].selector._temp
    ...
    def validation_step(self, batch, batch_idx):
        (y, y_loss), (y_hat, y_hat_loss), C, sizes, mask, _ = self.shared_step(
            batch, batch_idx
        )

        loss = self.compute_loss(
            y_hat_loss, y_loss, mask, sizes, batch.batch_size, name=f"val_loss"
        )
        # Logging
        # Compute metrics on base level
        ys = torch.split(y, sizes, dim=-2)
    ...
    def test_step(self, batch, batch_idx):
        ...
        loss = self.compute_loss(
            y_hat_loss, y_loss, mask, sizes, batch.batch_size, name=f"test_loss"
        )
        # Logging
    ...
    def on_train_epoch_start(self) -> None:
        ...
        if (
            self.current_epoch == self.reconciliation_start_epoch
            and self.reconcile_forecasts
        ):
